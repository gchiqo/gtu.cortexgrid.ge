...
// Cross-browser namespace: `browser.*` on Firefox, `chrome.*` on Chrome/Edge.
...
async function load() {
    const data = await ext.storage.local.get('lastCard');
    _card = data.lastCard;
...
function renderFound(card, matches) {
    ...
    // Build the "can't find? visit gtu.cortexgrid.ge" line as real DOM nodes
    // — Mozilla's AMO linter rejects any innerHTML assignment, even with
    // static, dictionary-controlled content.
    const cf = $('t_cantFind');
    cf.replaceChildren();
    cf.appendChild(document.createTextNode(pt('cant_find_prefix')));
    const cfLink = document.createElement('a');
    cfLink.href = 'https://gtu.cortexgrid.ge/';
    cfLink.target = '_blank';
    cfLink.rel = 'noopener';
    cfLink.textContent = 'gtu.cortexgrid.ge';
    cf.appendChild(cfLink);
